...
**A**: It creates a block-level container used to group content.
...
- **Attributes**: Elements can have attributes that modify their behavior. Example:

  ```html
  <p id="intro" class="text">Text</p>
  Example Question:
Q: What is the difference between the #title and .grid selector in CSS?
A: #title targets an element with the ID title, while .grid targets all elements with the class grid.
...
A: Padding is the space inside the border but outside the content, while the margin is the space outside the border.

4. CSS Flexbox and Grid
Flexbox:
Used for responsive designs where elements adjust based on container size.
Key Properties:
...
A: The images will be aligned based on the flex properties like flex-direction, justify-content, etc.
...
A: It defines a function that returns the string 'Hello'.
...
A: It selects the element by its ID and attaches an event listener that triggers when the event occurs.
...
A: Use document.getElementById("byu").style.color = 'green';.
...
rm: Remove files.
Example Question:
Q: What does the console command chmod, pwd, cd, ls, mv, rm do?
A: Each command manages file and directory manipulation in the terminal.
...
A: Yes, it ensures encrypted communication between a web server and a browser.
...
A: Depending on the state of the promise, it will output either a resolved value or a rejection.
